...
// Poll por software (replica el manejo del test, sin depender del IRQ pin).
uint8_t keypad_scanner_tca8418_poll()
{
    uint8_t int_stat = read_reg(TCA8418_REG_INT_STAT);
    uint8_t keycode  = 0;

    if (int_stat & 0x01) {  // evento de tecla
        keycode = read_reg(TCA8418_REG_KEY_EVENT_A);
        // si no quedan mas eventos, limpiar el flag de tecla
        if ((read_reg(TCA8418_REG_KEY_LCK_EC) & 0x0F) == 0) {
            write_reg(TCA8418_REG_INT_STAT, 1);
        }
    }
    if (int_stat & 0x02) {  // evento GPIO: leer y limpiar
        read_reg(TCA8418_REG_GPIO_INT_STAT_1);
        read_reg(TCA8418_REG_GPIO_INT_STAT_2);
        read_reg(TCA8418_REG_GPIO_INT_STAT_3);
        write_reg(TCA8418_REG_INT_STAT, 2);
