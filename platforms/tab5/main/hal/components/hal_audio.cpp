...
// SFX procedural "decoding": secuencia de bleeps de datos (estilo modem/handshake)
// para acompañar la intro de arranque. Generado en código (sin assets externos).
void HalEsp32::playDecodeSfx()
{
    const int sr    = 48000;
    const int durMs = 700;                   // corto (menos pico de corriente al arrancar)
    const int n     = sr * durMs / 1000;     // muestras mono
    const int seg   = sr * 26 / 1000;        // segmentos de ~26 ms
    std::vector<int16_t> buf((size_t)n * 2); // estereo intercalado

    unsigned int rng = 0xC0FFEE11u;
    auto rnd = [&]() { rng = rng * 1664525u + 1013904223u; return rng; };

    float phase = 0.f, freq = 900.f;
    int   segLeft = 0, segLen = seg;
    bool  silent = false;
    for (int i = 0; i < n; i++) {
        if (segLeft <= 0) {
            segLeft = segLen = seg + (int)(rnd() % seg);
            silent  = (rnd() % 5 == 0);             // ~20% silencio (huecos de datos)
            freq    = 300.f + (float)(rnd() % 2500);
            phase   = 0.f;
        }
        int pos = segLen - segLeft;
        segLeft--;
        int16_t s = 0;
        if (!silent) {
            phase += 2.f * 3.14159265f * freq / sr;
            float v = sinf(phase);
            v = 0.5f * v + 0.5f * (v >= 0 ? 1.f : -1.f);  // medio cuadrada -> mas "digital"
            // envolvente trapezoidal corta para evitar clicks
            float env = 1.f;
            int fade = 120;
            if (pos < fade) env = (float)pos / fade;
            else if (segLeft < fade) env = (float)segLeft / fade;
            s = (int16_t)(v * env * 4500.f);
        }
        buf[(size_t)i * 2]     = s;
        buf[(size_t)i * 2 + 1] = s;
    }
...
enum Mp3PlayTarget_t {
    ...
    MP3_PLAY_TARGET_FILE,   // fichero .mp3 de la SD (cargado a RAM)
};
...
struct MusicTestData_t {
    ...
    uint8_t* fileBuf                     = nullptr;  // buffer del .mp3 (PSRAM)
    size_t   fileSize                    = 0;
    std::string nowPlaying;                          // nombre del track actual
};
...
static void _music_play_task(void* param)
{
    ...
    switch (_music_test_data.target) {
        ...
        case MP3_PLAY_TARGET_SHUTDOWN_SFX:
            ...
            break;
        case MP3_PLAY_TARGET_FILE:
            mp3_size = _music_test_data.fileSize;
            if (_music_test_data.fileBuf && mp3_size > 0)
                fp = fmemopen((void*)_music_test_data.fileBuf, mp3_size, "rb");
            break;
    }
    ...
    if (_music_test_data.fileBuf) {  // liberar el .mp3 cargado en RAM
        free(_music_test_data.fileBuf);
        _music_test_data.fileBuf = nullptr;
        _music_test_data.fileSize = 0;
    }
    _music_test_data.nowPlaying.clear();
...
// --- Reproductor de ficheros .mp3 de la SD (Winamp simple) ---
bool HalEsp32::musicPlayFile(const std::string& path)
{
    {
        std::lock_guard<std::mutex> lock(_music_test_data.mutex);
        if (_music_test_data.state != hal::HalBase::MUSIC_PLAY_IDLE) return false;
    }
    // Cargar el .mp3 entero a RAM (PSRAM) y reproducir con fmemopen (no mantiene la SD).
    bsp_sdcard_init((char*)"/sd", 5);
    FILE* f = fopen(path.c_str(), "rb");
    if (!f) { bsp_sdcard_deinit((char*)"/sd"); return false; }
    fseek(f, 0, SEEK_END);
    long sz = ftell(f);
    fseek(f, 0, SEEK_SET);
    uint8_t* buf = nullptr;
    ...
    bool ok = buf && ((long)fread(buf, 1, sz, f) == sz);
    fclose(f);
    bsp_sdcard_deinit((char*)"/sd");
    if (!ok) { if (buf) free(buf); return false; }

    std::lock_guard<std::mutex> lock(_music_test_data.mutex);
    _music_test_data.fileBuf  = buf;
    _music_test_data.fileSize = (size_t)sz;
    auto p = path.find_last_of('/');
    _music_test_data.nowPlaying = (p == std::string::npos) ? path : path.substr(p + 1);
    try_create_music_play_task(MP3_PLAY_TARGET_FILE);
    return true;
}

void HalEsp32::musicStop()
{
    std::lock_guard<std::mutex> lock(_music_test_data.mutex);
    _music_test_data.killSignal = true;
}

bool HalEsp32::musicIsPlaying()
{
    std::lock_guard<std::mutex> lock(_music_test_data.mutex);
    return _music_test_data.state == hal::HalBase::MUSIC_PLAY_PLAYING;
}

std::string HalEsp32::musicNowPlaying()
{
    std::lock_guard<std::mutex> lock(_music_test_data.mutex);
    return _music_test_data.nowPlaying;
}
